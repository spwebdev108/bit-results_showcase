...
* **Premium Interactive UI**: Highly-responsive views styled with Google Fonts (Outfit), Font Awesome icons, and Bootstrap 5 glassmorphism.
* **Animated Student Records**: Beautiful table layouts with hover-sensitive transition animations mapping student details.
* **Student Avatars & Profiles**: Dedicated profile pictures rendered dynamically from database-stored image URLs (with automated fallback).
* **Detailed Academic Record Cards**: Dedicated profile pages showcasing individual student metrics, profile avatars, and Semester 1 & Semester 2 SGPA.
...
* **Icons & Typography**: [Font Awesome](https://fontawesome.com/) (v6.6.0) & [Google Fonts](https://fonts.google.com/) (Outfit)
...
| `image.url` | `String` | URL of the student profile image (with default placeholder profile image fallback). |
| `image.filename` | `String` | Reference filename for the student profile image (default: `"studentimage"`). |
